...
pub fn print_hex_table(gram_file_contents: &String,
binary_path: &String,
struct_offset: u64,
    ...
    match gram_parse::check_file_large_enough(struct_offset, parsed_gram.metadata.size, &mut binary_file, &binary_path) {
        Ok(_) => (),
        _ => return Err(TableResult::OffsetTooLarge)
    }
    let raw_data: Vec<u8> = binary_file.bytes().take(parsed_gram.metadata.size as usize).map(|r: Result<u8, _>| r.unwrap()).collect(); // change this into match?
    ...
    let hex_view = HexViewBuilder::new(&raw_data[..parsed_gram.metadata.size as usize]).address_offset(0x00).row_width(0x10).add_colors(color_vector).finish();
